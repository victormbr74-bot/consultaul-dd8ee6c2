...
export default function Backup4G() {
  ...
  const { data: lotericas = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Filter those with 4G SIM card info
      return all.filter((l: any) => {
        const raw = l.raw_data || {};
        return raw["SIM CARD 4G"] && String(raw["SIM CARD 4G"]).trim() !== "";
      });
    },
  });
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex items-center gap-2">
        ...
        <div>
          <h1 className="text-2xl font-bold text-foreground">Backup - 4G</h1>
          <p className="text-sm text-muted-foreground">Lotéricas com backup via SIM Card 4G</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="border-border/80">
          <CardContent className="p-4">
            <p className="text-[10px] uppercase tracking-wider text-muted-foreground">Total 4G</p>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-xs">SIM Card 4G</TableHead>
            ...
            <TableBody>
              {filtered.slice(0, 50).map((l: any) => (
                <TableRow key={l.cod_ul}>
                  ...
                  <TableCell className="text-xs font-mono">{(l.raw_data || {})["SIM CARD 4G"]}</TableCell>
